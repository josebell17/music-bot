...
class MusicBot(commands.Bot):
    ...
    async def setup_hook(self):
        # Khởi chạy Web Server ngầm phục vụ việc ping giữ server không bị ngủ
        self.loop.create_task(start_web_server())
        await self.tree.sync()
        print("Đã đồng bộ hóa các lệnh Slash và khởi chạy Web Server!")
...
async def handle(request):
    return web.Response(text="Bot đang online và hoạt động bình thường!")
